...
3. **Run all cells top to bottom.** Provide R2 credentials via Colab Secrets or Kaggle Secrets (`R2_MODELS_ACCOUNT_ID`, `R2_MODELS_ACCESS_KEY_ID`, `R2_MODELS_SECRET_ACCESS_KEY`, `R2_MODELS_BUCKET_NAME` — same values as `processing/`'s Railway env vars, see `.env.example`). The manual-paste fallback cell exists but secrets are strongly preferred — never commit real credentials into the notebook's saved output.
...
6. **Promote to live only if step 4 looks good, via `r2_release.py`** — the only path that also makes the version public:
   - `cd training && uv run python r2_release.py --model sentiment publish <version>` (needs the `R2_MODELS_*` env vars, e.g. from a local `.env` or exported in your shell, *and* an authenticated `gh` CLI with access to `goodgorithm/goodgorithm`).
